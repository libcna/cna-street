...
void PropFactory::driver(GeometryCollector& collector, Rng& rng, const Material* skin,
                         const Material* clothes, float shoulderHeight, float build,
                         float reach) const
{
    // Sitting, from the hips up. The origin is the seat cushion between the
    // hips; +Z is where the car is going, +X is the car's left, which is the
    // same convention every other prop here uses.
    //
    // Proportions from the seated posture rather than the standing one: the
    // shoulder is about 0.55 m over the cushion, the crown another 0.29 above
    // that, and the arm goes forward and down to a wheel rim a third of a
    // metre in front of the chest, bending at the elbow -- a straight rod
    // from shoulder to rim reads as a spoke, which is what the first version
    // of this looked like through a windscreen. Two material groups, skin and
    // clothes, so a driver is two draws.
    const float shoulder = std::max(shoulderHeight, 0.42f);
    const float halfWidth = 0.190f + 0.030f * build;
    const float halfDepth = 0.115f + 0.035f * build;
    const float lean = 0.10f;   ///< how far back into the seat, per metre of height

    MeshBuilder& coat = collector.builder(clothes);
    coat.setTileSize(0.45f);
    MeshBuilder& flesh = collector.builder(skin);
    flesh.setTileSize(0.25f);
    const auto back = [&](float y) { return -y * lean; };

    // The trunk: a reclined stack from the cushion to the shoulders, widest at
    // the chest.
    const float waistY = shoulder * 0.40f;
    coat.addEllipsoid(Vector3(0.0f, waistY, back(waistY)),
                      Vector3(halfWidth * 0.92f, waistY * 0.70f, halfDepth * 1.05f), 14, 8);
    coat.addEllipsoid(Vector3(0.0f, shoulder * 0.78f, back(shoulder * 0.78f)),
                      Vector3(halfWidth, shoulder * 0.32f, halfDepth), 14, 8);
    // The shoulder line, which is what a viewer reads first.
    coat.addCylinderBetween(Vector3(-halfWidth * 0.92f, shoulder, back(shoulder)),
                            Vector3(halfWidth * 0.92f, shoulder, back(shoulder)),
                            0.070f + 0.012f * build, 10, true);

    // Neck and head, the head turned a few degrees so a row of drivers is not
    // a row of mannequins.
    const float neckY = shoulder + 0.050f;
    flesh.addCylinderBetween(Vector3(0.0f, shoulder - 0.02f, back(shoulder)),
                             Vector3(0.0f, neckY, back(neckY) + 0.012f), 0.050f, 8, false);
    const float headY = neckY + 0.118f;
    const float turn = rng.range(-0.12f, 0.12f);
    const Vector3 head(std::sin(turn) * 0.012f, headY, back(headY) + 0.016f);
    flesh.addEllipsoid(head, Vector3(0.084f, 0.110f, 0.096f), 14, 10);
    // Hair over the crown and the back of the skull only: an ellipsoid as big
    // as the head swallows the face and reads as a helmet.
    coat.addEllipsoid(Vector3(head.X, head.Y + 0.030f, head.Z - 0.024f),
                      Vector3(0.082f, 0.080f, 0.082f), 12, 8);

    // Both arms, shoulder to elbow to a hand on the rim. The elbow is out and
    // down, the forearm comes back in toward the wheel's centre.
    for (const float side : {-1.0f, 1.0f})
    {
        const Vector3 from(side * halfWidth * 0.86f, shoulder - 0.04f, back(shoulder));
        const Vector3 elbow(side * (halfWidth * 1.02f), shoulder - 0.235f,
                            back(shoulder) + reach * 0.32f);
        const Vector3 wrist(side * (halfWidth * 0.62f), shoulder - 0.150f, reach);
        coat.addCylinderBetween(from, elbow, 0.058f + 0.008f * build, 8, true);
        coat.addCylinderBetween(elbow, wrist, 0.048f + 0.006f * build, 8, true);
        flesh.addEllipsoid(wrist, Vector3(0.040f, 0.038f, 0.055f), 8, 6);
    }
}
